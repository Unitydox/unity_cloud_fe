...
const EmptyAvatar = ({ size = 'medium' }: Pick<AvatarProps, 'size'>) => (
  <span
    ...
    className={classNames(
      'inline-block overflow-hidden bg-gray-100 rounded-full cursor-pointer',
      sizes[size],
    )}
  >
    <svg
...
export default function Avatar({ size = 'medium', src, alt }: AvatarProps) {
  ...
  return (
    <img
      className={classNames('inline-block rounded-full cursor-pointer object-cover', sizes[size])}
      src={src}
      alt={alt}
    />
  )
